...
def main():
	...
	while not found:
		...
		for _ in range(no_of_fireflies - 2):
			FF_x.append(numpy.random.randint(-99, 99)) # grid(x = -10, y = 60) then randint(x-1, y-1) = (-9, 59)
			FF_y.append(numpy.random.randint(-99, 99))
		...
		if path == "Route Not Possible":
			no_of_fireflies = no_of_fireflies + 50
		else:
			found = True
			Final_x = FF_x.copy()
			Final_y = FF_y.copy()
